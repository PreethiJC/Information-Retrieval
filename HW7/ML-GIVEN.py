...
def createPerformanceFile(testDict):
    with open( 'RankingGIVEN.csv', 'w') as csvfile:
...
def getBestFeatures(X, y, csvFile, names):
    ...
    names = dataset.columns[0:len(names)-5].values[selector.get_support()]
...
def getAccuracy(txtFile, csvFile):
    ...
    X = array[:,0:lenNGrams-4] #contains only retrieval model scores
    Y = array[:,lenNGrams]
    print(Y)
