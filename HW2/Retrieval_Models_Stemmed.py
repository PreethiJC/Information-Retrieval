...
def Total_okapiTF(qNo, termVector, termStats, docInfo):
    ...
    with open('Files/Stemmed/OkapiTF_Results_File.txt', 'a+') as queryResults:
        rank = 1
        for ds in docScore:
...
def Okapi_BM25(qNo, termVector, termStats, docInfo):
    ...
    docScore.sort(key=itemgetter(1), reverse=True)
    with open('Files/Stemmed/OkapiBM25_Results_File.txt', 'a+') as queryResults:
        rank = 1
        for ds in docScore:
            queryResults.write('%s Q0 %s %d %lf Exp\n' % (qNo, ds[0], rank, ds[1]))
            if rank == 1000:
...
def UnigramLM_Laplace(qNo, catalog, termMap, docMap):
    ...
    with open('Files/Stemmed/UnigramLMLaplace_Results_File.txt', 'a+') as queryResults:
        rank = 1
        for ds in DocScore:
...
def proximity(qNo, termVector, termStats, docInfo):
    ...
    docScore.sort(key=itemgetter(1), reverse=True)
    with open('Files/Stemmed/Results/Proximity_Results_File.txt', 'a+') as queryResults:
        rank = 1
        for ds in docScore:
            queryResults.write('%s Q0 %s %d %lf Exp\n' % (qNo, ds[0], rank, ds[1]))
            if rank == 1000:
...
for qNo in qNums:
    i += 1
    termStats = unpickler('Files/Stemmed/Pickles/termStats_Proximity%s.p' % i)
    termVector = unpickler('Files/Stemmed/Pickles/termVector_Proximity%s.p' % i)
